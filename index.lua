...
if game:HttpGet(ui_lib) then
    ...
    local Main = int:CreatePage({
      Subject = "Main",
      Footer = ""
    })

    local Settings = int:CreatePage({
      Subject = "Settings",
      Footer = ""
    })
    ...
    local Utility = Main:CreateSection({
      Header = "Utility"
    })
 
    local Misc = Main:CreateSection({
      Header = "Misc"
    })

    -- Settings Sections

    local AimlockSettings = Settings:CreateSection({
      Header = "Aimlock"
    })

    local FlySettings = Settings:CreateSection({
      Header = "Fly"
    })

    local NoclipSettings = Settings:CreateSection({
      Header = "Noclip"
    })

    local UISettings = Settings:CreateSection({
      Header = "CONFIG"
    })

    --// Main Tab
    
    ui_storage.Aimlock = Combat:CreateCheck({
      Text = "Aimlock",
    ...
    ui_storage.Noclip = Combat:CreateCheck({
      Text = "Noclip",
    ...
    ui_storage.Fly = Combat:CreateCheck({
      Text = "Fly",
    ...
    TeleportBox = Misc:CreateTextBox({
      Text = "Teleport",
    ...
    UISettings:CreateButton({
      Text = "Save Settings",
      Script = function() SaveSettings() end
    })
...
RunService.RenderStepped:Connect(function(...)
    ...
    if not ui_settings["Utility"]["Target ESP"] or aimlock_settings["AimlockTarget"] == nil or not ui_settings["Combat"]["Aimlock"] then
    ...
    if aimlock_settings["AimlockTarget"] ~= nil and ui_settings["Utility"]["Target ESP"] and ui_settings["Combat"]["Aimlock"] then
       if aimlock_settings["AimlockPart"] then
            ...
            for z, x in pairs(BoxTable) do
               if x.Player ~= aimlock_settings["AimlockTarget"]  then
                  if x.Box ~= nil then x.Box:Remove(); end
                  table.remove(BoxTable, z)
               end
            end
            ...
            for z, x in pairs(BoxTable) do
               if x.Player == aimlock_settings["AimlockTarget"] then
                  ...
                  if PlayerModel == nil then table.remove(BoxTable, z) end
               end
            end
            
            local TargetBox = esp_lib:Add(aimlock_settings["AimlockTarget"].Character.HumanoidRootPart, {
...
RunService.RenderStepped:Connect(function(...)
    if ui_settings["Utility"]["ESP"] then
       ...
       for z, x in pairs(ValidPlayers) do
          ...
          local TargetBox = esp_lib:Add(x.HumanoidRootPart, {
...
local Hooking = function(self, ...) 
   ...
   if Method == "FireServer" then
      if self.Name == ("MainEvent") then
         if table.find(Events, Args[1]) then return wait(9e9) end
         if Args[1] == ("UpdateMousePos") and ui_settings["Combat"]["Aimlock"] and aimlock_settings["AimlockTarget"] ~= nil then
...
StartFLY = function(FlyPart)
    ...
    FlyPart.ChildRemoved:Connect(function(Child)
	...
	UserInputService.InputBegan:Connect(function(Input, G)
	    if Input.KeyCode == Enum.KeyCode[ui_settings["Keys"]["Fly"]] and ui_settings["Combat"]["Fly"] then
