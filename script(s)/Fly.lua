...
local UpdateLowerTorso = function(Weld)
    pm_lib.Player().CharacterAdded:Connect(
        function(Character)
...
local AddFly = function(Part)
            ...
            function()
                repeat
                    ...
                    if pm_lib.Humanoid() ~= nil and pm_lib.Humanoid() ~= false then
                        pm_lib.Humanoid().PlatformStand = true
                    end
    ...
    flyKeyDown =
        ...
        function(KEY)
            ...
            pcall(
                function()
                    workspace.CuerrentCamera.CameraType = Enum.CameraType.Track
                end
            )
        end
    )
    ...
    task.spawn(
        function()
            repeat
                wait()
            until not in_lib.GetValue(_G.MetaWareSettings, 0, "FlyToggled", "Combat")
            FLYING = false
        end
    )

    if in_lib.GetValue(_G.MetaWareSettings, 0, "FlyToggled", "Combat") then
        FLY()
    end
end

local Part = CreateGyroPart()
AddFly(Part)
return true
